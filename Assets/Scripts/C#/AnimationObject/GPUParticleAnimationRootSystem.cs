...
using UnityEngine;
public enum ComputeAnimationType
{
    None,
    Setup,
    SetupImmediately,
...
public enum UpdateAnimationMethodType
{
    None,
    Target,
    Explode,
...
public interface GPUAnimationParticleTarget
{
    Mesh Mesh { get; }
    int VertexCount { get; }
...
public interface GPUAnimationParticleTargetGroup
{
    Texture2DArray TextureArray { get; }
    Matrix4x4[] MatrixData { get; }
    AnimationInitData[] AllInitData { get; }
    uint[] Indices { get; }
    void Initialize(GPUParticleAnimationRootSystem system);
    void UpdateMatrices();
}

public struct AnimationTransformParticle
{
    public int isActive;
    public int targetId;
    public Vector2 uv;
...
public struct AnimationInitData
{
    public int isActive;
    public Vector3 targetPosition;
...
public class GPUParticleAnimationRootSystem : MonoBehaviour
{
    ...
    private AnimationTransformParticle[] _particleData = null;
    private AnimationInitData[] _initDataList = null;
    Vector4[] _speres;
    ...
    public void SetGroup(AnimationGPUParticleGroups group, AnimationGPUParticleSubGroups subGroup = null)
    {
        DisableAllParticles();
    ...
    public void ChangeUpdateMethod(UpdateAnimationMethodType type)
    {
        switch (type)
        {
            case UpdateAnimationMethodType.Target:
                _currentUpdateKernel = _kernelUpdateAsTarget;
                break;

            case UpdateAnimationMethodType.Explode:
                _currentUpdateKernel = _kernelUpdateAsExplosion;
                break;

            case UpdateAnimationMethodType.Gravity:
                _currentUpdateKernel = _kernelUpdateAsGravity;
                break;
            case UpdateAnimationMethodType.KeepPosition:
                _currentUpdateKernel = _kernelUpdateKeepAsPosition;
                break;
            case UpdateAnimationMethodType.UpdatePosition:
                _currentUpdateKernel = _kernelUpdateAsPosition;
                break;
        }
    }

    public void ChangeUpdateMethodWithClear(UpdateAnimationMethodType type, bool needsImmediately = false)
    {
        ChangeUpdateMethod(type);
        ClearMatrices();
        DisableAllParticles();

        ComputeAnimationType computeType = needsImmediately ? ComputeAnimationType.SetupImmediately : ComputeAnimationType.Setup;
    ...
    public void DisableAllParticles()
    {
        ...
        Dispatch(ComputeAnimationType.DisableAll);
    }


    public void UpdateInitData(AnimationInitData[] updateData)
    {
    ...
    /// </summary>
    public void UpdateMatrices(AnimationGPUParticleGroups group)
    {
    ...
    public void UpdateAllBuffers(ComputeAnimationType type)
    {
    ...
    public void Dispatch(ComputeAnimationType type)
    {
        int kernelId = GetKernelID(type);
    ...
    private int GetKernelID(ComputeAnimationType type)
    {
        switch (type)
        {
            case ComputeAnimationType.Setup:
                return _kernelSetupParticles;

            case ComputeAnimationType.SetupImmediately:
                return _kernelSetupParticlesImmediately;

            case ComputeAnimationType.DisableAll:
    ...
    private void CreateBuffers()
    {
        ...
        _particleData = new AnimationTransformParticle[_maxCount];
        for (int i = 0; i < _particleData.Length; i++)
        {
            _particleData[i] = new AnimationTransformParticle
            {
        ...
        _initDataList = new AnimationInitData[_initDataCount];
