...
[System.Serializable]
public struct AnimationIndexData
{
    public int ID;
    public uint[] Indices;
}

public class AnimationGPUParticleGroups : MonoBehaviour, GPUAnimationParticleTargetGroup
{
    ...
    private AnimationParticleTarget[] _particleTargets = null;
    private Vector3[] _allVertices = null;
    private AnimationInitData[] _allInitData = null;
    ...
    public AnimationInitData[] AllInitData => _allInitData;
    ...
    public void Initialize(GPUParticleAnimationRootSystem system)
    {
        if (_autoDetection)
        {
            _particleTargets = GetComponentsInChildren<AnimationParticleTarget>(true);
        }
        else
        {
            _particleTargets = _targets.Select(t => t.GetComponent<AnimationParticleTarget>()).ToArray();
        }
        ...
        _allInitData = new AnimationInitData[GetCount()];
    ...
    private void CollectAllData()
    {
        ...
        for (int i = 0; i < _particleTargets.Length; i++)
        {
            AnimationParticleTarget t = _particleTargets[i];
